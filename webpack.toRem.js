...
Multipage.prototype.px2rem = function (numPx) {
...
Multipage.prototype.handlePx2Rem = function (filePath) {
    ...
    return new Promise((r, j) => {
        fs.readFile(filePath, 'utf8', function (err, files) {
            ...
            while(whileRet = result.match(/(border\W*:\W*)(\d+\.*\d*)(rem)/)){
                result = result.replace(new RegExp('('+whileRet[2]+')(rem)'), self.rem2px(whileRet[2]) + 'px');
            }
            fs.outputFileSync(filePath, result);
            if (err) {
                j(err);
